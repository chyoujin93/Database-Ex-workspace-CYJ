...
WHERE deptno = 20
ORDER BY deptno;
--> 사원번호가 다른 2명: 부서번호, 급여가 동일 -> 하나로 줄어듦
...
--3. LIKE 연산자, 와일드카드 
--EMP테이블에서, 조건: 사원명이 S로 끝나는 데이터를 출력
SELECT * from emp
WHERE ename like '%S';

--4. EMP테이블에서, 부서번호가 30이고, 직무가 SANESMAN인 데이터를 출력 (AND)
SELECT * from emp
where deptno = 30 AND job = 'SALESMAN';

--5. EMP테이블에서, 부서번호 20 또는 30이고, 급여가 2000 초과인 데이터 출력
-- OR 대신에 IN 사용을 권장. 
SELECT * from emp
where deptno IN(20,30) AND SAL > 2000;

--6. EMP테이블에서, 급여가 2000에서 3000사이인 데이터 출력 (BETWEEN)
SELECT * from emp
where SAL BETWEEN 2000 AND 3000;

--6. 동일한 결과값을 출력. 비트윈 대신에 비교 연산자로 풀이.
SELECT * from emp
where SAL < 2000 OR SAL > 3000 ;

--7. EMP테이블에서, 사원명에 E가 들어가고, 부서번호가 30이면서 
-- 급여가 1000~2000가 아닌 데이터를 출력 (LIKE연산자, BETWEEN 사용)
SELECT * from emp
WHERE (ENAME LIKE '%E%' AND DEPTNO = 30)
AND SAL NOT BETWEEN 1000 AND 2000;

--7. 동일한 결과값을 출력. 비트윈 대신 비교 연산자로 풀이.
SELECT * from emp
WHERE (ENAME LIKE '%E%' AND DEPTNO = 30)
AND (SAL < 1000 OR SAL > 2000);

--7-2. 위와 동일한 조건으로, 부서번호가 30인 경우도 조건에 포함.
SELECT * from emp
WHERE ENAME LIKE '%E%' 
AND (SAL < 1000 OR SAL > 2000)
AND DEPTNO = 30;

--8. EMP테이블에서, 상여금이 없고 직무가 매니저 또는 클럭인 데이터, 그리고 
-- 사원명의 두번째 알파벳이 L인 데이터를 출력. (IS NULL, AND사용)
SELECT * from emp
WHERE (COMM IS NULL
AND JOB IN('MANAGER', 'CLERK'))
AND NOT ENAME LIKE '_L%';

--9. 기존 사원명과 사원명을 대문자, 소문자, 첫문자만 대문자로 각각 표시.
SELECT ENAME,
UPPER(ENAME), LOWER(ENAME), INITCAP(ENAME)
FROM EMP;

--10. EMP테이블, 사원명 'scott' 문자 변경하지 않고 조회.
...
--> 조건만 lower, upper일 뿐, 출력값은 변경되지 않음.

-------------------------------------------
--11. EMP테이블, ENAME에 'sc'이란 문자가 포함된 데이터를 조회.
--단, 문자를 변경하지 않고
SELECT ENAME FROM EMP
WHERE ENAME LIKE UPPER('%sc%');

--12. dual테이블은 오라클에서 제공하는 시스템 테이블. (테스트용)
SELECT 10, 20, deptno from emp
where deptno = 20;

SELECT upper('%sc%') FROM dual;


--13. emp테이블에서 이름은 소문자, 직책은 대문자로 조회.ALTER
SELECT lower(ename) "ENAME(소문자)", upper(job) "JOB(대문자)" from emp;

--length함수에 대해서
SELECT LENGTH('%sc%')+8 as len from dual;

--14. emp테이블에서 이름과 이름의 길이를 표시하고, 
...
-- 사원명, 사원명길이, 5개 이하인 경우만 출력
SELECT ENAME, LENGTH(ename) from emp
WHERE length(ename) <= 5;

-- lengthb는 ㅎ ㅏ ㄴ ㄱ ㅡ ㄹ 이렇게 센다. 
